...
export const POST = requireAdmin(async (req: NextRequest, { params }: { params: Promise<{ id: string }> }) => {
  try {
    ...
    // Atomically increment allWinnersPosterCount
    const updatedProgram = await Program.findByIdAndUpdate(
      id,
      {
        $inc: { allWinnersPosterCount: 1 }
      },
      { new: true }
    );
    ...
    console.error('Error tracking all-winners poster:', error);
    return NextResponse.json({ error: 'Failed to track all-winners poster generation' }, { status: 500 });
  }
});
